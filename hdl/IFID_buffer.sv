module IFID_buffer(
	...
	input logic clk, rst_n, IFID_flush, IFID_write,
	output logic [31:0] pc_addr_o, inst_data_o
	);
	
	always_ff @(posedge clk) begin
		if(!rst_n || IFID_flush) begin // if reset enabled, flush buffers
			pc_addr_o <= 0;
			inst_data_o <= 0;
		end
		else if (!IFID_write) begin // else send input data through to output on the next clock cycle
			pc_addr_o <= pc_addr_i;
