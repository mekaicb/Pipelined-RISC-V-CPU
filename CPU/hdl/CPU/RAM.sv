module RAM(
	...
	always_comb begin
		...
		if(memread) begin 
			data_temp <= mem_array[eff_addr[13:2]]; // Simple read 
		...
		if(memwrite) begin // Write can be complicated
			case(byte_en)
				2'b00 : begin
					case(byte_sel)
						2'b00 : mem_array[eff_addr[13:2]][7:0] <= data_i[7:0];
						2'b01 : mem_array[eff_addr[13:2]][15:8] <= data_i[7:0];
						2'b10 : mem_array[eff_addr[13:2]][23:16] <= data_i[7:0];
						2'b11 : mem_array[eff_addr[13:2]][31:24] <= data_i[7:0];
					endcase
				end
				2'b01 : begin
					case(byte_sel)
						2'b00 : mem_array[eff_addr[13:2]][15:0] <= data_i[15:0];
						2'b01 : mem_array[eff_addr[13:2]][31:16] <= data_i[15:0];
					endcase
				end
				2'b10 : begin
					mem_array[eff_addr[13:2]] <= data_i;
				end
				default : mem_array[eff_addr[13:2]] <= data_i;
			endcase
		end	
	end
	...
	always_comb begin // Handles the slicing and extension of the word read from memory
		case(byte_en_r)
			3'b000 : begin // Will be selecting one byte
				case(byte_sel_r) // Which one byte to select specfically from the word in memory?
					2'b00 : data_o = {24'b0, data_temp[7:0]}; // Select the first byte of the word from the memory array, zero extend
					2'b01 : data_o = {24'b0, data_temp[15:8]};
			...
			3'b001 : begin // Select two bytes
				case(byte_sel_r) // Which two bytes to select specifically
					2'b00 : data_o = {16'b0, data_temp[15:0]};
					2'b10 : data_o = {16'b0, data_temp[31:16]};
			...
			3'b011 : begin
				case(byte_sel_r)
					2'b00 : data_o = {{24{data_temp[7]}}, data_temp[7:0]}; // Sign extend the byte to 32 bits
					2'b01 : data_o = {{24{data_temp[15]}}, data_temp[15:8]}; // Select the 2nd byte, place as lowest byte and sign extend to 32 bits
			...
			3'b100 : begin
				case(byte_sel_r)
					2'b00 : data_o = {{16{data_temp[15]}}, data_temp[15:0]}; // Sign extend the halfword to 32 bits
					2'b01 : data_o = {{16{data_temp[31]}}, data_temp[31:16]}; // Select the 2nd halfword, sign extend to 32 bits
